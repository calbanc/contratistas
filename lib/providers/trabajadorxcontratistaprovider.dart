...
set isLoading(bool value) {
  ...
  Future<SyncEntregasResponse>sync(EntregaResponse entrega)async{
    ...
    final Map<String,dynamic>data={'IdCartilla':entrega.idcartilla,'Qr':entrega.qr,"Fecha":entrega.fecha,"Cantidad":entrega.cantidad,"Nombre_trabajador":entrega.nombre_trabajador,"IdEntrega":entrega.identrega};
    String parametros=json.encode(data);
    final url = Uri.https(_baseUrl, _endpoint);
    Map<String, String> header = new Map();
    header["content-type"] =  "application/x-www-form-urlencoded";
 ...
 Future<List<Trabajador>?> gettrabajadorbyqr(String qr)async{
  ...
  Future<List<EntregaResponse>?>entrega(EntregaResponse entrega)async{
    final db=await database;
  ...
  Future<int>insertentry(EntregaResponse entry)async{
    final db=await database;
    final res=await db.insert('ENTREGAS',entry.toJson());
    return res;
  }
  Future<int>updateentry(EntregaResponse entry) async {
    final db=await database;
    int cantidad=entry.cantidad!;
    int? identrega=entry.identrega;
    final res=await db.rawUpdate(''' 
      UPDATE ENTREGAS SET CANTIDAD= '$cantidad' , SwSincronizado='0' WHERE IdEntrega='$identrega'
    ''');
    return res;
...
Future<int>updateSync(String identrega) async {
  ...
  Future<List<ListadoEntregaResponse>?> listadoentregas(String idcartilla) async{
    ...
    final res=await db.rawQuery(''' 
     ...
     SELECT C.Cuartel,C.Labor,T.Nombre,SUM(E.CANTIDAD) AS 'Cantidad',e.IdCartilla,e.IdEntrega
      FROM ENTREGAS E
      INNER JOIN CARTILLAS C ON E.IdCartilla=C.IdCartilla
      INNER JOIN TRABAJADORESXCONTRATISTA T ON T.Qr=E.Qr
      where e.IdCartilla='$idcartilla'
